...
class buff {
    public:
        ...
        void setbuff(double ada,double apa,double aca,double pca,double dda,double adm,double apm,double acm,double pcm,double ddm,double turn){
            adAdd = ada;
            apAdd = apa;
            acAdd = aca;
            pcAdd = pca;
...
void buffmanage(character& chrac){
    int i = 0;
    for(i=0;i<chrac.buffpack.size();i++){
        if(chrac.buffpack[i].turns >= 0){
            chrac.buffpack[i].turns -= 1;
        }
        else{
            rent(i,chrac.buffpack);
        }
    }
}
//////初始化技能///////////////////////////////
class sheild : public skill{
    ...
    void use(character& player,character& anm){
        buff sheildbuff;
        sheildbuff.setbuff(0,0,50,0,0,1,1,1,1,1,3);
...
class bingfengzhanchi : public skill{
    public:
    bingfengzhanchi() {
        setskillname("冰凤展翅", "使自己的敏捷提高10,持续3回合");
    }
    void use(character& player,character& anm){
        buff bingfengbuff;
        bingfengbuff.setbuff(0,0,0,0,8,1,1,1,1,1,1);
...
class normalattack : public skill{
    ...
    void use(character& player,character& anm){
        anm.hp -= 1.0*player.allad();
        cout<<anm.name<<"受到了"<<1.0*player.allad()<<"点伤害"<<endl;
    }
};
//容器内部查找////////////////////////////////////////////////////
template <typename t>
...
void character::showcharacter(){
    cout<<name<<" 生命值 = "<<hp<<" 攻击力 = "<<ad<<"法强 = "<<ap<<" 护甲 = "<<ac<<"魔抗 = "<<pc<<" 闪避 = "<<dd<<endl;
}
...
double character::alldd(){
    ...
    for(i2=0;i<buffpack.size();i++){
        sum += buffpack[i].ddAdd;
    }
    for(i2=0;i<buffpack.size();i++){
        sum *= buffpack[i].ddMul;
    }
    return sum;
}
//总攻击力计算
double character::allad(){
    ...
    for(i2=0;i<buffpack.size();i++){
        sum += buffpack[i].adAdd;
    }
    for(i3=0;i<buffpack.size();i++){
        sum *= buffpack[i].adMul;
    }
    return sum;
}
//总法强
double character::allap(){
    ...
    for(i=0;i<warmon.size();i++){
        ...
        for(i2=0;i<buffpack.size();i++){
        sum += buffpack[i].apAdd;
    }
    for(i3=0;i<buffpack.size();i++){
        sum *= buffpack[i].apMul;
    }
    return sum;
}
//总护甲计算
double character::allac(){
    ...
    for(i=0;i<warmon.size();i++){
        ...
        for(i2=0;i<buffpack.size();i++){
        sum += buffpack[i].acAdd;
    }
    for(i3=0;i<buffpack.size();i++){
        sum *= buffpack[i].acMul;
    }
    return sum;
}
//总魔抗计算
double character::allpc(){
    ...
    for(i=0;i<warmon.size();i++){
        ...
        for(i2=0;i<buffpack.size();i++){
        sum += buffpack[i].pcAdd;
    }
    for(i3=0;i<buffpack.size();i++){
        sum *= buffpack[i].pcMul;
    }
    return sum;
}
...
void reallbattle(character& player,character& anm){
        ...
        while(check(player,anm)){
            cout<<"你的回合！"<<endl<<endl;
            this_thread::sleep_for(chrono::seconds(1));
            cout<<"1.普通攻击  2.查看背包 3.释放技能 4.查看敌人"<<endl<<endl;
            string re;
            cin>>ws;
            getline(cin,re);
            if(re=="1"){
